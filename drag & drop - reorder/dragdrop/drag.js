...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		hover_color = '#EFEFEF',	// (string) hover color
    ...
    var	drop_option = 'switch';	    // (string) drop_option has the following options: multiple, single, switch, switching and overwrite
    else
    var	drop_option = 'switching';

	var delete_cloned = true,		// (boolean) delete cloned div if the cloned div is dragged outside of any table
	...
	handler_onmousedown = function (e) {
		...
		if (div_drag !== obj.container) {
			div_drag = obj.container;
			init_tables();
			calculate_cells();
		}
	...
	clone_object = function () {
		...
		cloned_id[obj.id] += 1;
		// I assumed that custom property "container" will be automatically cloned - but not(?!)
		// this is reference to the DIV id="drag"
		obj_new.container = obj.container;
